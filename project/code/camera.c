...
uint8 camera_processing(uint16 row, uint16 row_total, uint16 row_black_pix_count, uint16 colum, uint16 colum_total, uint16 colum_black_pix_count)
{
    ...
    return camera_image_check_jump_strict(image_copy, row, row_total, row_black_pix_count, colum, colum_total, colum_black_pix_count);  // ¼ì²éÊÇ·ñÌøÔ¾£¨Ä¿Ç°¿ÉÒÔË®Æ½ºÍ´¹Ö±¼ì²é£©
