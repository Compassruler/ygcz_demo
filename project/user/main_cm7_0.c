...
// 视觉模式按照数组中的先后顺序执行，VISION_IDLE 不参与流程排序
...
static uint8 vision_plan_index = 0;  // 当前执行的模式数组下标
...
int main(void)
{
  ...
  vision_mode_plan_reset();                             // 视觉模式数组初始化
    //==========================
    // 变量
    //==========================
    int i = 0;
...
//        }
        ...
        if(!pause_flag)
        {
            ...
            if (vision_phase_done_flag)
            {
                ...
                vision_mode_plan_move_next();  // 进入数组中的下一视觉阶段
...
// =========================================================== 视觉部分 ==========================================================
     ...
     if (pause_flag == false)
    {
        vision_detect_mode = vision_mode_plan_get_current();  // 按数组顺序选择视觉模式
    }
    else
    {
        vision_detect_mode = VISION_IDLE;  // 空闲
    }
    appipc_send_core0_data((uint16)fabsf(car_speed), (uint8)vision_detect_mode, vision_phase_bab, vision_binary_threshold, vision_bump_finish);  // 发送车速、视觉状态、统一二值化阈值和积分完成标志到核1
